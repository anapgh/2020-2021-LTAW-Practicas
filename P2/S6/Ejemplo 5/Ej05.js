...
const EJ5_HTML = fs.readFileSync('Ej05.html','utf-8');
...
const server = http.createServer((req, res) => {
  ...
  if (cookie) {
    ...
    //-- Parseamos por si hay muchos ejemplos, ya que se separan por ;
    ...
    pares.forEach((element, index) => {
      ...
      //-- De esta manera al tener 2 elementos, lo asigno asi
      let [nombre, valor] = element.split('=');

      //-- Leer el usuario
      //-- Solo si el nombre es 'user'
      //-- El .trim se come a los espacios
      if (nombre.trim() === 'user') {
        user = valor;
      }
    });
